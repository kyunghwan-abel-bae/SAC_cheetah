...
import torch
import numpy as np
from my_sac_cheetah import SAC, seed_all
import time
...
def visualize_trained_agent(model_path, num_episodes=3):
    ...
    hidden_dims = (256, 256, ) # my_sac_cheetah.py와 동일하게 설정
    ...
    # 에이전트 생성 및 모델 로드
    agent = SAC(state_dim, action_dim, hidden_dims=hidden_dims)
    try:
        checkpoint = torch.load(model_path)
        # 'actor_state_dict' 대신 'policy_state_dict'를 사용
