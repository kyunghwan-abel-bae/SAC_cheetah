...
class MLPGaussianPolicy(nn.Module):
    ...
    def sample(self, state):
        ...
        action_scale = self.action_scale.to(action.device)
        action_bias = self.action_bias.to(action.device)
...
class SAC:
    def __init__(
        ...
        self.replay_buffer = ReplayBuffer(buffer_size, min_buffer_size)
    ...
    def step(self, transition, step_count):
        ...
        if step_count % self.actor_update_freq == 0:
    ...
    def save_model(self, path='saved_models'):
        ...
        }, os.path.join(path, 'sac_cheetah_periodic_model.pth'))
        
        print(f"Model saved to {os.path.join(path, 'sac_cheetah_periodic_model.pth')}")
...
if __name__ == "__main__":
    ...
    batch_size = 2048
    gamma = 0.99
    actor_update_freq = 1024 # 주기적 업데이트 빈도
    ...
    for t in tqdm(range(1, max_iterations + 1)):
        ...
        result = agent.step((s, a, r, s_prime, terminated), t)
        s = s_prime
        ...
        if t % eval_intervals == 0:
            ...
            agent.save_model(f'saved_models/checkpoint_periodic_{t}')
    
    agent.save_model('saved_models/final_model_periodic')
    ...
    plt.tight_layout()
    plt.savefig('training_results_periodic.png')
    plt.show()
